...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        if (customView) {
            ...
            if ([self.emptyDataSetSource respondsToSelector:@selector(borderColorForEmptyDataSet:)]) {
...
- (void)setupConstraints
{
    ...
    if (_customView) {
        ...
        if ([self canShowButton]) {
            ...
            CGFloat padding = ([UIScreen mainScreen].bounds.size.width-150)/2.0;
            [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"H:|-%f-[button(>=0)]-%f-|",padding,padding]
                                                                                     options:0 metrics:metrics views:views]];
        }
        // or removes from its superview
        else {
        ...
        for (int i = 0; i < subviewStrings.count; i++) {
            ...
            if ([string isEqualToString:@"button"]) {
                [verticalFormat appendFormat:@"[%@(30)]", string];
            } else {
